...
export default function CountryAnalysis() {
  ...
  useEffect(() => {
    const handleDemoSelect = (event) => {
      ...
      setTimeout(() => setSelectedCountry('india'), 1000);
    };
  ...
  const vaccChartData = useMemo(() => {
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      data.push({ day: i, current: currentTraj, boosted: boostedTraj });
    }
    return data;
  ...
  const fetchCountryData = async (country) => {
    ...
    try {
      ...
      if (loadRes) setHospitalLoadData(loadRes);
    } catch (err) {
  ...
  const updateSimulation = (val) => {
    ...
    const cData = fakeBase.map((item, i) => {
      if (i >= 23 && forecastData[i - 23]) {
        return { ...item, predicted: forecastData[i - 23].predicted_cases || item.predicted };
      }
      return item;
    });
    const newData = cData.slice(23).map((d, i) => {
      ...
      return { ...d, baseline: basePred, simulated: Math.floor(basePred * extraEffect) };
    });
    setSimChartData(newData);
  };

  const filteredCountries = Object.entries(COUNTRY_STATIC).filter(
    ([key, data]) => data.name.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="absolute top-[calc(100%+6px)] left-0 right-0 z-50 bg-white dark:bg-[#0f172a] border border-slate-300 dark:border-slate-600/50 rounded-[10px] overflow-hidden shadow-xl max-h-[240px] overflow-y-auto">
        ...
        {filteredCountries.length > 0 ? (
          ...
          <div className="px-4 py-8 flex flex-col items-center gap-2 text-center">
            ...
            >
                ...
                <div className="h-[240px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={fullComposedData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                      ...
                      <XAxis dataKey="day" tick={{ fill: '#7a8499', fontSize: 11, fontFamily: 'DM Sans' }} axisLine={false} tickLine={false} tickCount={10} />
                      <YAxis tick={{ fill: '#7a8499', fontSize: 11, fontFamily: 'DM Sans' }} axisLine={false} tickLine={false} tickFormatter={(val) => `${(val/1000).toFixed(1)}K`} />
                      <Tooltip contentStyle={customTooltipStyle} formatter={(val, name) => [`${(val/1000).toFixed(1)}K cases`, name === 'actual' ? 'Actual' : 'Predicted']} labelFormatter={(lbl) => `Day ${lbl}`} />
              ...
              <div className="col-span-1 bg-white dark:bg-[#0f172a] p-5 rounded-[14px] border border-slate-200 dark:border-slate-700/50 hover:border-slate-300 transition-colors flex flex-col gap-4">
                ...
                <div className="flex-1 flex flex-col items-center justify-center pt-2">
                  <div className={clsx("text-[48px] font-semibold font-bold leading-none tracking-tight", cR0 > 1.0 ? "text-red-500" : "text-success")}>
                    <CountUp to={cR0} isDecimals={true} colorClass="inherit" />
                  </div>
                ...
                <div className="h-[100px] w-full mt-auto">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                      <Line type="monotone" dataKey="r0Trend" stroke={cR0 > 1.0 ? "#ef4444" : "#10b981"} strokeWidth={2} dot={false} />
                    ...
                    <div className="h-[220px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={mortalityData.monthly} margin={{ top: 10, right: 0, left: -10, bottom: 0 }}>
                          ...
                          <Tooltip contentStyle={customTooltipStyle} formatter={(value, name) => [value.toLocaleString(), name]} cursor={{ fill: 'rgba(0,0,0,0.02)' }} />
            ...
            {/* 8. VACCINATION IMPACT SIMULATOR CARD */}
            <motion.div
